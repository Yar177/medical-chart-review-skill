...
The cross-cutting HEDIS extraction-patterns reference lives in the sibling `hedis-nlp` skill's `references/nlp/extraction-patterns.md` (in this same repo). Most of that content (section detection, abbreviation expansion, doc-type classification, copy-forward detection) applies here unchanged; this file calls out HCC-specific extensions.
...
Same patterns as HEDIS (see the sibling `hedis-nlp` skill's `references/nlp/extraction-patterns.md`). HCC-specific notes:
...
- Sibling `hedis-nlp` skill, `references/nlp/extraction-patterns.md` - shared HEDIS extraction patterns
