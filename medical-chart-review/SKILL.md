...
5. **Cross-reference.** Reconcile problem list ↔ assessment ↔ med list ↔ billed codes ↔ labs. Discrepancies are findings. For every dated evidence row, validate the date kind, section, tense, and signer per `references/date-of-service.md` before accepting it.
...
- **Date of service (DoS) taxonomy, note-type → DoS mapping, section validity, tense/modality lexicon, precision rules, copy-forward (clinical view)** → `references/date-of-service.md`
