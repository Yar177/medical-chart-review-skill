...
> **Deep dives:** This file is the auditor-oriented overview. For NLP engineering on per-measure denominator / numerator / exclusion / NLP-signal cards, date-of-service rules, assertion handling, extraction patterns, evaluation, the HEDIS abstraction worksheet, and supplemental-data provenance, see the sibling `hedis-nlp` skill in this repo (it has its own `references/hedis/`, `references/nlp/`, `references/hedis-supplemental-data.md`, and `templates/hedis-abstraction.md`).
...
### Hybrid measures
Some HEDIS measures allow **hybrid** (admin + medical record) sampling - chart review fills gaps that claims miss. Hybrid abstraction is subject to NCQA Medical Record Review Validation (MRRV); see the sibling `hedis-nlp` skill's `references/hedis-supplemental-data.md` for provenance rules and audit-trail expectations.

### Per-measure deep-dive cards

The sibling `hedis-nlp` skill's `references/hedis/` directory has per-measure reference cards covering denominator, numerator, exclusions, NLP signal phrases, common documentation gaps, and notes. Cards are organized by clinical area:
...
See the sibling `hedis-nlp` skill's `references/hedis/README.md` for the full index and card structure.
